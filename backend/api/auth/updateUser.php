...
$roleRaw = $input['role'] ?? ($input['user_role'] ?? '');
$role = strtolower((string) $roleRaw);
$role = in_array($role, ['admin', 'kitchen'], true) ? $role : '';
$password = $input['password'] ?? '';
$confirmPassword = $input['confirm_password'] ?? '';

if ($id <= 0 || $role === '') {
    Response::json(['success' => false, 'error' => 'Invalid payload'], 422);
    exit;
}
...
try {
    ...
    $userModel->updateAdminRole($id, $role);
